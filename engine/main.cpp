...
// NOTE: Text chunking and the Ollama client used to live here in C++.
// Both moved to the Go gateway (gateway/ollama.go, gateway/chunk.go) —
// this engine now only ever receives embeddings that are already computed,
// and only ever returns embeddings/ids/distances. It never talks to Ollama.

// =====================================================================
//  DOCUMENT DATABASE  — HNSW over externally-computed embeddings
// =====================================================================
...
int main(int argc, char** argv) {
    // Internal-only port — the Go gateway is the sole intended caller.
    // Bind to 127.0.0.1 by default so the engine isn't reachable off-box.
    int port = 9090;
    std::string bindHost = "127.0.0.1";
    if (argc > 1) port = std::atoi(argv[1]);
    if (const char* envPort = std::getenv("ENGINE_PORT")) port = std::atoi(envPort);
    if (const char* envHost = std::getenv("ENGINE_HOST")) bindHost = envHost;
    ...
    std::cout << "=== VectorDB Engine (internal, C++) ===" << std::endl;
    std::cout << "listening on " << bindHost << ":" << port << std::endl;
    std::cout << db.size() << " demo vectors | " << DIMS << " dims | HNSW+KD-Tree+BruteForce" << std::endl;
    std::cout << "No Ollama / RAG / HTTP-public logic here — that lives in the Go gateway." << std::endl;
    ...
    // ── DOCUMENT ENGINE ENDPOINTS (internal, called only by the Go gateway) ──
    // These never touch Ollama or do text chunking — they only index and
    // search whatever embedding vector they're handed. Chunking and
    // embedding happen in Go (gateway/chunk.go, gateway/ollama.go).

    // POST /docengine/insert  {"title":"...","text":"...","embedding":[...]}
    svr.Post("/docengine/insert", [&](const httplib::Request& req, httplib::Response& res) {
        cors(res);
        auto title = extractStr(req.body, "title");
        auto text  = extractStr(req.body, "text");
        std::vector<float> emb;
        {
            size_t p = req.body.find("\"embedding\"");
            if (p != std::string::npos) {
                p = req.body.find('[', p);
                size_t e = req.body.find(']', p);
                if (p != std::string::npos && e != std::string::npos)
                    emb = parseVec(req.body.substr(p + 1, e - p - 1));
            }
        }
        if (title.empty() || emb.empty()) {
            res.set_content("{\"error\":\"need title, text, and embedding\"}",
                            "application/json"); return;
        }
        int id = docDB.insert(title, text, emb);
        res.set_content("{\"id\":" + std::to_string(id) +
                        ",\"dims\":" + std::to_string(docDB.getDims()) + "}",
                        "application/json");
    });

    // DELETE /docengine/delete/123
    svr.Delete(R"(/docengine/delete/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        cors(res);
        int id  = std::stoi(req.matches[1]);
    ...
    // GET /docengine/list
    svr.Get("/docengine/list", [&](const httplib::Request&, httplib::Response& res) {
        cors(res);
        auto docs = docDB.all();
        std::ostringstream ss; ss << '[';
    ...
    // POST /docengine/search  {"embedding":[...],"k":3}
    // Returns id/title/text/distance for the top-k nearest chunks — the
    // gateway builds the RAG prompt itself, this just does the retrieval.
    svr.Post("/docengine/search", [&](const httplib::Request& req, httplib::Response& res) {
        cors(res);
        ...
        if (emb.empty()) {
            res.set_content("{\"error\":\"need embedding\"}", "application/json"); return;
        }
    ...
    // GET /docengine/stats
    svr.Get("/docengine/stats", [&](const httplib::Request&, httplib::Response& res) {
        cors(res);
        std::ostringstream ss;
    ...
    svr.listen(bindHost.c_str(), port);
    return 0;
}
